...
**v1 — breadth and robustness.** PDF and image/scan ingestion (AI-assisted extraction + OCR), self-service mapping-template creation, multi-currency and FX gain/loss, corporate actions, average-cost as a selectable method, price/FX snapshots for unrealized-gain valuation, dividend withholding reconciliation, the full dashboard set, import/tax-year notifications, admin triage tooling, and a second and third country tax module to actually prove the plugin architecture generalizes.
